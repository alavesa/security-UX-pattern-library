...
function AIDisclosureDemo() {
  ...
  const sendMessage = () => {
    ...
    const responseIndex = Math.min(messages.filter(m => m.role === "ai").length, responses.length - 1);

    setTimeout(() => {
      setMessages(prev => [...prev, { role: "ai", text: responses[responseIndex] }]);
      setTyping(false);
    }, 1500);
  };
    ...
    <div className="w-full max-w-lg">
      ...
      <div className="bg-white rounded-2xl shadow-lg border-2 overflow-hidden" style={{ borderColor: view === "dark" ? "#fca5a5" : "#86efac" }}>
        ...
        {view === "ethical" && (
          <div className="bg-purple-50 border-b border-purple-200 px-4 py-2 flex items-start gap-2">
            ...
            <p className="text-xs text-purple-800">
              ...
              <a href="#" onClick={e => e.preventDefault()} className="text-purple-600 underline ml-1">Learn more</a>
            </p>
          </div>
        )}

        {/* Chat */}
        <div ref={scrollRef} className="p-4 space-y-3 min-h-[200px] max-h-[300px] overflow-y-auto">
          {messages.length === 0 && (
        ...
        <div className="border-t border-gray-200 px-4 py-3 flex gap-2">
          ...
          />
          <button onClick={sendMessage} className="text-blue-600 bg-transparent border-none cursor-pointer">
