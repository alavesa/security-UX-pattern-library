    ...
    return (
      <div className="w-full max-w-sm">
        <div className="rounded-2xl p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
          ...
          <h2 className="text-xl font-bold font-mono mb-1" style={{ color: "var(--green)" }}>Welcome back!</h2>
          <p className="text-sm font-mono mb-2" style={{ color: "var(--text)" }}>Signed in as <strong style={{ color: "var(--text-bright)" }}>{email || "user"}</strong></p>
          <div className="text-xs font-mono space-y-1 mb-4" style={{ color: "var(--text-dim)" }}>
            <p>Session created at {new Date().toLocaleTimeString()}</p>
            <p>IP: 192.168.1.*** (partially masked)</p>
            {rememberMe && <p style={{ color: "var(--green)" }}>Remember me: device trusted for 30 days</p>}
          </div>
          <div className="rounded-lg p-3 text-left mb-4" style={{ background: "rgba(0,229,255,0.05)", border: "1px solid rgba(0,229,255,0.2)" }}>
            <p className="text-xs font-mono" style={{ color: "var(--text)" }}>
              <strong style={{ color: "var(--cyan)" }}>Security UX note:</strong> The success screen shows partial session info for transparency, masks the full IP for privacy, and confirms "remember me" status so users understand the security implications.
            </p>
          </div>
          <button onClick={reset} className="text-sm font-mono bg-transparent border-none cursor-pointer" style={{ color: "var(--text)" }}>
    ...
    return (
      <div className="w-full max-w-sm">
        <div className="rounded-2xl p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
          ...
          <h2 className="text-xl font-bold font-mono mb-1" style={{ color: "var(--text-bright)" }}>Verify your identity</h2>
          <p className="text-sm font-mono mb-6" style={{ color: "var(--text)" }}>We sent a code to your authenticator app</p>
          <div className="flex gap-2 justify-center mb-4">
            {[0, 1, 2, 3, 4, 5].map(i => (
              <input
                ...
                className="w-10 h-12 text-center text-lg font-mono rounded-lg focus:outline-none"
          ...
          {mfaError && (
            <div className="flex items-start gap-2 text-sm font-mono p-3 rounded-lg mb-4" role="alert" style={{ background: "rgba(255,51,51,0.08)", border: "1px solid rgba(255,51,51,0.2)", color: "var(--red)" }}>
          ...
          <p className="text-xs font-mono mt-4" style={{ color: "var(--text-dim)" }}>
            This step appears because the account has MFA enabled — a critical security measure.
          </p>
        </div>
        <button onClick={reset} className="mt-4 text-xs font-mono mx-auto block bg-transparent border-none cursor-pointer" style={{ color: "var(--text)" }}>
          Reset demo
        </button>
      </div>
    );
  ...
  return (
    <div className="w-full max-w-sm">
      <div className="rounded-2xl p-8" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        <h2 className="text-xl font-bold font-mono text-center mb-1" style={{ color: "var(--text-bright)" }}>Welcome back</h2>
        <p className="text-sm font-mono text-center mb-6" style={{ color: "var(--text)" }}>Sign in to your account</p>

        {/* Social login */}
        <div className="space-y-2 mb-5">
          <button
            onClick={handleSocialLogin}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium font-mono transition-colors cursor-pointer"
          ...
          <button onClick={handleSocialLogin} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium font-mono transition-colors cursor-pointer" style={{ background: "transparent", color: "var(--text-bright)", border: "1px solid var(--border)" }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="email" className="block text-sm font-medium font-mono mb-1" style={{ color: "var(--text-bright)" }}>Email</label>
            <input
              id="email"
              type="email"
              value={email}
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label htmlFor="password" className="text-sm font-medium font-mono" style={{ color: "var(--text-bright)" }}>Password</label>
              <button type="button" onClick={() => {}} className="text-xs font-mono bg-transparent border-none cursor-pointer p-0" style={{ color: "var(--green)" }}>
                Forgot password?
              </button>
            </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm font-mono" style={{ color: "var(--text-bright)" }}>Remember this device</span>
            <span className="text-xs font-mono" style={{ color: "var(--text-dim)" }}>(30 days)</span>
          </label>

          {error && (
            <div className="flex items-start gap-2 text-sm font-mono p-3 rounded-lg" role="alert" style={{ background: "rgba(255,51,51,0.08)", border: "1px solid rgba(255,51,51,0.2)", color: "var(--red)" }}>
              <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
              <span>{error}</span>
            </div>
          )}
          ...
          <button
            ...
            className="w-full py-2.5 rounded-lg font-medium font-mono text-sm disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 border-none cursor-pointer"
            style={{ background: "var(--green)", color: "var(--bg)" }}
          >
        ...
        <p className="text-xs font-mono text-center mt-4" style={{ color: "var(--text)" }}>
          Don't have an account? <a href="#" onClick={e => e.preventDefault()} className="hover:underline" style={{ color: "var(--green)" }}>Sign up</a>
        </p>

        {locked && (
          <p className="text-xs font-mono text-center mt-2" style={{ color: "var(--red)" }}>
            Account locked
          </p>
        )}
      </div>
