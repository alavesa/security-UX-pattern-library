    ...
    return (
      <div className="w-full max-w-sm">
        <div className="rounded-2xl border p-8 text-center">
          ...
          <p className="text-sm mb-2">Signed in as <strong>{email || "user"}</strong></p>
          <div className="text-xs space-y-1 mb-4">
            <p>Session created at {new Date().toLocaleTimeString()}</p>
            <p>IP: 192.168.1.*** (partially masked)</p>
            {rememberMe && <p >Remember me: device trusted for 30 days</p>}
          </div>
          ...
          <button onClick={reset} className="text-sm hover: bg-transparent border-none cursor-pointer">
    ...
    return (
      <div className="w-full max-w-sm">
        <div className="rounded-2xl border p-8 text-center">
          ...
          <p className="text-sm mb-6">We sent a code to your authenticator app</p>
          <div className="flex gap-2 justify-center mb-4">
            {[0, 1, 2, 3, 4, 5].map(i => (
              <input
                ...
                className="w-10 h-12 text-center text-lg font-mono border rounded-lg focus:outline-none focus:ring-2"
                value={mfaCode[i]}
                onChange={e => {
          ...
          {mfaError && (
            <div className="flex items-start gap-2 text-sm p-3 rounded-lg mb-4" role="alert">
      ...
      <div className="rounded-2xl border p-8">
        ...
        <div className="space-y-2 mb-5">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 border py-2.5 rounded-lg text-sm font-medium hover: transition-colors cursor-pointer"
          >
          ...
          <button className="w-full flex items-center justify-center gap-2 border py-2.5 rounded-lg text-sm font-medium hover: transition-colors cursor-pointer">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:"
              disabled={locked}
              autoComplete="email"
            />
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              ...
              <button type="button" onClick={() => {}} className="text-xs hover: bg-transparent border-none cursor-pointer p-0">
                Forgot password?
              </button>
            </div>
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 pr-10 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:"
                disabled={locked}
          ...
          {error && (
            <div className={`flex items-start gap-2 text-sm p-3 rounded-lg ${locked ? " " : " "}`} role="alert">
              <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
              <span>{error}</span>
            </div>
          )}
          ...
          <button
            ...
            className="w-full text-white py-2.5 rounded-lg font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
          >
        ...
        <p className="text-xs text-center mt-4">
          Don't have an account? <a href="#" onClick={e => e.preventDefault()} className="hover:">Sign up</a>
        </p>
